...
const FilterDate = ({
  ...
  const changeDateFilter = (e) => {
    ...
    setDateList((prev) =>
      prev.map((date) => ({ ...date, checked: date.requestValue === newDate }))
    );
  ...
  return (
    <div className="release-date">
      ...
      <select
        className="release-date__select"
        name="date"
        onChange={changeDateFilter}
        value={selectedValue.requestValue}
      >
        {dateList.map((date) => (
          <option
            className="release-date__option"
            value={date.requestValue}
            key={date.id}
          >
            {date.description}
          </option>
