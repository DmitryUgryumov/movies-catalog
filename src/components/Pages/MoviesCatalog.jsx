...
const checkProperty = (key, nonKeyValue = "") =>
  sessionStorage.getItem(key)
    ? JSON.parse(sessionStorage.getItem(key))
    : nonKeyValue;

const initialStateFilter = (filtersArr, typeFilter, filterActive) =>
  typeFilter === "radio"
    ? filtersArr.map((item) => ({
        ...item,
        checked: item.requestValue === filterActive,
      }))
    : filtersArr.map((item) => ({
        ...item,
        checked: filterActive.includes(item.requestValue),
      }));
...
const MoviesCatalog = () => {
  ...
  const [sortedActive, setSortedActive] = useState(
    checkProperty("sorted", "popularity.desc&vote_count.gte=50")
  );

  const [genresList, setGenresList] = useState(
    initialStateFilter(genresArr, "checkbox", genresActive)
  );
  const [dateList, setDateList] = useState(
    initialStateFilter(dateArr, "radio", dateActive)
  );
  const [sortedList, setSortedList] = useState(
    initialStateFilter(sortedArr, "radio", sortedActive)
  );
  ...
  useEffect(
    () => (document.documentElement.scrollTop = checkProperty("scroll", 0))
  );
  ...
  const saveScrolling = (e) => {
    if (
      e.target.tagName !== "LI" &&
      e.target.className !== "movie-card__date"
    ) {
    ...
    <>
      <div className="filters">
        <Sorted
          ...
          setSortedActive={setSortedActive}
